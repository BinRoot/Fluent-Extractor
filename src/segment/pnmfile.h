...
static image<rgb> *loadMat(const cv::Mat& img) {
	image<rgb> *im = new image<rgb>(img.cols, img.rows);
	//std::cout << img.cols << " " << img.rows << std::endl;

	for (int j = 0; j < img.rows; j++) {
		for (int i = 0; i < img.cols; i++) {
			unsigned char b = img.at<cv::Vec3b>(j, i)[0];
			unsigned char g = img.at<cv::Vec3b>(j, i)[1];
			unsigned char r = img.at<cv::Vec3b>(j, i)[2];
			im->access[j][i] = {r, g, b};
		}
	}
...
static cv::Mat saveMat(image<rgb> *im) {
	cv::Mat mat(im->height(), im->width(), CV_8UC3, cv::Scalar(0,0,255));
	for (int j = 0; j < im->height(); j++) {
		for (int i = 0; i < im->width(); i++) {
			rgb c = im->access[j][i];
			cv::Vec3b vec(c.b, c.g, c.r);
			mat.at<cv::Vec3b>(j, i) = vec;
		}
	}
	return mat;
}
